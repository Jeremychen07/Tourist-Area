$(function(){
  ...
  let bigindata = document.querySelector("#begindate"); //開始日期
  let enddate = document.querySelector("#enddate"); //結束日期  
  ...
  function setBackgroud(){
    //使用者背景顏色變更
    let inputs = document.querySelector("#chooseBackgroud");
  ...
  //處理資料函式
  function dealConatiner(data){
    container.innerHTML = "";
    const picturl="./sea.jpg"; //picturl預設
    for(let i = 0; i < data.length; i++)
    {
        ...
        `
        <div class='icon'>
          <div class='pic' style='background-image: url(${info.Picture1 != "" ? info.Picture1 : picturl})'></div>
        ...
        <div class='top'>
          ...
          <div class='adr'>
            ${info.Town}-${info.Add}
            <br>
            <div class='notic'>${info.Cycle}</div>
          </div>
          <div class='time'>
            <i class="fas fa-bell">
              <small style="color: #EB5E00"> ${info.Start.substr(0,10)}</small> 
              - 
              <small style="color: #EB5E00">${info.End.substr(0,10)} </small>
            </i>
          </div>
          <div class='text'>
            說明:
